...
client_kafka.topics
topic = client_kafka.topics['myproject-kafka']
	...
	if request.method == 'POST':
		if form.validate_on_submit():
			...
			if users.find_one({"email": email}) is None:
				flash("Sorry, this user does not exists!")
				with topic.get_sync_producer() as producer:
					producer.produce(bytes(str('User with ' + email + ' does not exists!'), encoding='utf-8'))
			else:
				if users.find_one({"password": passmd5.upper()}) is None:
					flash("Wrong password!")
					with topic.get_sync_producer() as producer:
						producer.produce(bytes(str('Wrong password for ' + email + ' !'), encoding='utf-8'))
				else:
					check_status = 0
					...
					if check_status == 4:
						...
						with topic.get_sync_producer() as producer:
							producer.produce(bytes(str('User ' + email + ' can not log in, because his account is banned!'), encoding='utf-8'))
					else:
						flash("Success!")
						with topic.get_sync_producer() as producer:
							producer.produce(bytes(str('User ' + email + ' log in!'), encoding='utf-8'))
						users.update_one({
							"email": email},{
		...
		if form.validate_on_submit():
			...
			if users.find_one({"email": email}) is None:
				...
				if password != confirm_password:
					...
					with topic.get_sync_producer() as producer:
						producer.produce(bytes(str('While registration the user with ' + email + ' entered different passwords'), encoding='utf-8'))
				else:
					last_id = 0
					...
					with topic.get_sync_producer() as producer:
						producer.produce(bytes(str('User ' + email + ' has been succesfully registered!'), encoding='utf-8'))
					return redirect('login')
			else:
				flash("Sorry, the user with this email is already registered!")
				with topic.get_sync_producer() as producer:
					producer.produce(bytes(str('The user with ' + email + ' is already registered'), encoding='utf-8'))
	...
	if request.method == 'POST':
		if form1.validate_on_submit():
			...
			if pages.find_one({"title": title}) is None:
				...
				flash("Page added!")
				with topic.get_sync_producer() as producer:
						producer.produce(bytes(str('Page with title ' + title + ' added!'), encoding='utf-8'))
				
				return redirect('mongopages')
			else:
				flash("Sorry, the page with this title is already exists!")
				with topic.get_sync_producer() as producer:
					producer.produce(bytes(str('Page with title ' + title + ' is already exists!'), encoding='utf-8'))
		...
		if form2.validate_on_submit():
			...
			if pages.find_one({"page_id": page_id_del}) is None:
				...
				with topic.get_sync_producer() as producer:
					producer.produce(bytes(str('There is no page with ID ' + str(page_id_del)), encoding='utf-8'))
			else:
				pages.delete_one({"page_id": page_id_del})
				access.delete_many({"page_id": page_id_del})
				flash("Page deleted.")
				with topic.get_sync_producer() as producer:
					producer.produce(bytes(str('Page with id ' + str(page_id_del) + ' deleted'), encoding='utf-8'))
...
def data():
	...
	ufull_path = '/home/elizaveta/myproject/app/content/users.png'
	pfull_path = '/home/elizaveta/myproject/app/content/pages.png'
	
	with open(ufull_path, "rb") as ufilestring:
		uconv = ufilestring.read()
	udata = base64.b64encode(uconv)
	udata = udata.decode("utf-8")
	
	with open(pfull_path, "rb") as pfilestring:
		pconv = pfilestring.read()
	pdata = base64.b64encode(pconv)
	pdata = pdata.decode("utf-8")
	
	return render_template('data.html', title='Data', udata=udata, pdata=pdata)
...
def editpage(title):
	...
	flash("Page edited!")
	with topic.get_sync_producer() as producer:
		producer.produce(bytes(str('Page with title ' + title + ' edited!'), encoding='utf-8'))
...
def deletefile(title, filename):
	...
	flash("File deleted.")
	with topic.get_sync_producer() as producer:
		producer.produce(bytes(str('File ' + filename + ' from ' + title + ' deleted'), encoding='utf-8'))
...
def addfile(title):
	...
	if filename !=  '':
		if filename not in arrayfiles:
			...
			flash("File added!")
			with topic.get_sync_producer() as producer:
				producer.produce(bytes(str('File ' + filename + ' added!'), encoding='utf-8'))
		else: 
			flash('This file is already on page!')
			with topic.get_sync_producer() as producer:
				producer.produce(bytes(str('File ' + filename + ' is already on page ' + title + '!'), encoding='utf-8'))
	else:
		flash("No file selected!")
		with topic.get_sync_producer() as producer:
			producer.produce(bytes(str('No file selected for loading on ' + title + '!'), encoding='utf-8'))
...
def editaccess(title):
	...
	if request.method == 'POST':
		if form.validate_on_submit():
			...
			flash("Access edited!")
			with topic.get_sync_producer() as producer:
				producer.produce(bytes(str('Access for page ' + title + ' edited!'), encoding='utf-8'))
